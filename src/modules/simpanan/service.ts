...
function toPostgresInteger(value: number | bigint) {
...
function transactionError(error: unknown) {
  if (
    ...
    || error instanceof InvalidPaymentAccountError
    || error instanceof InvalidAccountError
    || error instanceof AmountOverflowError
    || error instanceof HargaSahamNotFoundError
    || error instanceof InsufficientBalanceError
    || error instanceof MutasiSimpananNotFoundError
    || error instanceof MutasiSimpananAlreadyProcessedError
    || error instanceof MutasiSimpananNotDeletedError
...
export const SimpananService = {
  ...
  getMutasi: Effect.fn('SimpananService.getMutasi')(function* (
    userId: number,
    isAdmin: boolean,
  ...
  createSetoran: Effect.fn('SimpananService.createSetoran')(function* (
    userId: number,
  ...
  createPenarikan: Effect.fn('SimpananService.createPenarikan')(function* (
    userId: number,
  ...
  deleteMutasi: Effect.fn('SimpananService.deleteMutasi')(function* (
    userId: number,
    ids: number[],
  ) {
    return yield* Effect.tryPromise({
  ...
  approveMutasi: Effect.fn('SimpananService.approveMutasi')(function* (
    id: number,
    adminId: number,
  ) {
    return yield* Effect.tryPromise({
      try: () =>
        db.transaction(async (tx) => {
          ...
          if (invalidAccountId !== undefined) {
            throw new InvalidAccountError({ akunId: invalidAccountId })
          }
          ...
          if (target.jenisSimpanan === 'tabungan') {
            ...
            const nominalTotal = toPostgresInteger(amounts.nilaiNominal)
            const details = [
              {
                jurnalId: createdJournal.id,
                akunId: target.akunId,
  ...
  rejectMutasi: Effect.fn('SimpananService.rejectMutasi')(function* (
    ...
    if (!rejectionReason) {
      return yield* new InvalidRejectionReasonError()
    }
